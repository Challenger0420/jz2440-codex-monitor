...
入口是 `src/oabi_start.S` 的 `_start`；链接时只使用 `src/oabi_runtime.c`、`src/codex_monitor.c` 和 compiler support library `-lgcc`。OABI syscall number 和调用约定集中在 `src/oabi_runtime.c`。
...
```bash
bash scripts/build-target.sh
```

两套脚本共同读取 `scripts/target-flags.txt`，因此 CFLAGS/LDFLAGS 不在两个脚本中分叉。脚本只生成 `build/codex-monitor-oabi`，并在构建前删除该目标的本地中间文件；不会访问开发板。

目标检查使用 ARM 交叉工具链自带程序，而不是 Windows 原生 binutils：

```bash
bash scripts/inspect-target.sh build/codex-monitor-oabi
```
...
```text
arm-linux-gnueabi-gcc -march=armv4t -mcpu=arm920t -marm -mabi=apcs-gnu -mfloat-abi=soft -Os -Wall -Wextra -std=c99 -ffreestanding -fno-builtin -fno-stack-protector -c src/oabi_start.S -o build/oabi_start.o
arm-linux-gnueabi-gcc -march=armv4t -mcpu=arm920t -marm -mabi=apcs-gnu -mfloat-abi=soft -Os -Wall -Wextra -std=c99 -ffreestanding -fno-builtin -fno-stack-protector -nostdlib -nodefaultlibs -nostartfiles -Wl,--no-warn-mismatch -Wl,-e,_start -o build/codex-monitor-oabi build/oabi_start.o src/oabi_runtime.c src/codex_monitor.c -lgcc
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\inspect-target.ps1
```
...
bash scripts/build-target.sh                         # Build A
sha256sum build/codex-monitor-oabi > build/sha256-build-1.txt
bash scripts/inspect-target.sh build/codex-monitor-oabi
bash scripts/build-target.sh                         # Build B
