...
public static class ProtocolParserTests
{
    public static int Run()
    {
        ...
        if (!ProtocolParser.IsRequest("<CQMREQ|V=1>\n") ||
            ProtocolParser.IsRequest("<CQMREQ|V=2>\n") ||
            ProtocolParser.IsRequest("<CQMREQ>\n"))
            throw new InvalidOperationException("request frame test failed");
        ...
        extracted = ProtocolParser.ExtractFrames("<CQMREQ|V=1>\n");
        if (extracted.Count != 1 || !ProtocolParser.IsRequest(extracted[0]))
            throw new InvalidOperationException("request extraction test failed");
        passed += 6;
