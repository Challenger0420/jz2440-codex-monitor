...
#include "../device/include/ui_layout.h"
#include "font_data.h"
                        ...
                        uint32_t target_now, uint32_t black, uint32_t text)
{
    ...
    if (!state->valid) {
        draw_text(fb, HEADER_STATUS_X, HEADER_STATUS_Y, &font_label, "WAITING FOR DATA...",
                  HEADER_TEXT_SPACING,
    ...
    clock_text[11] = '\0';
    if (target_now >= state->last_receive_target && target_now - state->last_receive_target > 120U)
        stale = 1;
    if (stale) draw_text(fb, HEADER_STATUS_X, HEADER_STATUS_Y, &font_label, "STALE",
                         HEADER_TEXT_SPACING,
                         pack_rgb(&fb->var, COLOR_WARNING_R, COLOR_WARNING_G, COLOR_WARNING_B));
    draw_text(fb, HEADER_CLOCK_X - measure_text(&font_label, clock_text, HEADER_TEXT_SPACING),
              HEADER_CLOCK_Y, &font_label, clock_text, HEADER_TEXT_SPACING, text);
}
                        ...
                        uint32_t target_now)
{
    ...
    int footer_value_x, footer_text_y, footer_value_y;
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (byte == '\n') {
            if (!*overflow && *length > 0) {
                ...
                if (parse_quota_frame(frame, *length, state, target_now)) draw_screen(fb, state, target_now);
            }
            *length = 0; *overflow = 0;
...
int main(int argc, char **argv)
{
    ...
    char frame[256];
    int length = 0, overflow = 0, last_stale = -1;
    uint32_t target_now = board_time_seconds(), last_header_minute = 0;
    ...
    for (;;) {
        ...
        if (state.valid) {
            if (target_now >= state.sync_target_time) display_time = state.sync_timestamp + target_now - state.sync_target_time;
            if (target_now >= state.last_receive_target && target_now - state.last_receive_target > 120U) stale = 1;
            if (display_time / 60U != last_header_minute || stale != last_stale) {
                uint32_t black = pack_rgb(&fb.var, COLOR_BG_R, COLOR_BG_G, COLOR_BG_B),
                         text = pack_rgb(&fb.var, COLOR_TEXT_R, COLOR_TEXT_G, COLOR_TEXT_B);
                draw_header(&fb, &state, target_now, black, text);
                last_header_minute = display_time / 60U; last_stale = stale;
            }
        }
