...
& $Compiler @cflags -I (Join-Path $Root "apps\codex-monitor\target\include") -c (Join-Path $Root "platform\target\runtime\oabi_start.S") -o (Join-Path $BuildDir "oabi_start.o")
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Compiler @cflags @ldflags -I (Join-Path $Root "apps\codex-monitor\target\include") -o (Join-Path $BuildDir "codex-monitor-oabi") `
    (Join-Path $BuildDir "oabi_start.o") `
    (Join-Path $Root "platform\target\runtime\oabi_runtime.c") `
    (Join-Path $Root "apps\codex-monitor\target\src\codex_monitor.c") -lgcc
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
